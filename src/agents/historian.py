...
class Historian(Agent):
    def __init__(self, name="Dr. White", use_api=False):
        super().__init__(name, expertise="History", use_api=use_api)
    ...
    def generate_argument(self, topic):
        ...
        try:
            ...
            if not context or "No information found" in context:
                context = "Use historical knowledge about the topic."
            prompt = (
                f"You are a historian. Provide a concise argument on {topic}, "
                f"drawing on historical events, trends, or parallels. "
                f"Avoid repetition and ensure historical accuracy. Context: {context[:500]}"
            )
