...
class Ethicist(Agent):
    def __init__(self, name="Dr. Brown", use_api=False):
        super().__init__(name, expertise="Ethics", use_api=use_api)
    ...
    def generate_argument(self, topic):
        ...
        try:
            ...
            if not context or "No information found" in context:
                context = "Use ethical principles to discuss the topic."
            prompt = (
                f"You are an ethicist. Provide a concise argument on {topic}, "
                f"applying ethical frameworks like utilitarianism, deontology, or virtue ethics. "
                f"Avoid repetition and focus on moral implications. Context: {context[:500]}"
            )
