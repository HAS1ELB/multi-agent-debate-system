...
from src.knowledge.data_sources.pubmed import PubMed
from src.utils.cache import cache

class Scientist(Agent):
    def __init__(self, name="Dr. Smith", use_api=False):
        super().__init__(name, expertise="Science", use_api=use_api)
        self.pubmed = PubMed()
    ...
    def generate_argument(self, topic):
        ...
        try:
            ...
            pubmed_context = self.pubmed.search(topic)
            if pubmed_context and "Error" not in pubmed_context:
                context = context + " " + pubmed_context if context else pubmed_context
            if not context or "No information found" in context:
                context = "Use scientific knowledge about the topic."
            prompt = (
                f"You are a scientist. Provide a concise, evidence-based argument on {topic}, "
                f"focusing on peer-reviewed data, measurements, or scientific consensus. "
                f"Avoid speculation and repetition. Context: {context[:500]}"
            )
